...
namespace agora_gaming_rtc
{
    public class AgoraJson
    {
        ...
        {
            var jData = JsonDocument.Parse(new string(data, 0, Array.IndexOf(data, '\0'))).RootElement;
            var jValue = jData.GetProperty(key);
            return typeof(T).Name switch
            {
                "Boolean" => jValue.GetBoolean(),
        ...
        {
            var jValue = JsonDocument.Parse(new string(data, 0, Array.IndexOf(data, '\0'))).RootElement.ToString();
        ...
        {
            var jValueArray = JsonDocument.Parse(new string(data, 0, Array.IndexOf(data, '\0'))).RootElement
                .GetProperty(key);
